...
namespace DanbooruDownloader.Utilities
{
    ...
    {
        public static readonly Dictionary<string, string> dataBaseFields = new()
        {
        ...
        public static void TryCreateTable(SqliteConnection connection)
        {
            using (SqliteTransaction transaction = connection.BeginTransaction())
            {
                ...
                string[] fields = dataBaseFields
                    .Select((field) => field.Key + " " + field.Value)
                    .ToArray();
                string fieldsSQl = string.Join(",\n", fields);
                fieldsSQl = "CREATE TABLE IF NOT EXISTS posts ( " + fieldsSQl + " );";
                command.CommandText = fieldsSQl;
        ...
        public static void InsertOrReplace(SqliteConnection connection, IEnumerable<JObject> posts)
        {
            using (SqliteTransaction transaction = connection.BeginTransaction())
            {
                foreach (JObject post in posts)
                {
                    ...
                    List<JProperty> properties = post.Properties().Where(p => dataBaseFields.ContainsKey(p.Name)).ToList();
